...
cssharplastbuild=$(curl --connect-timeout 10 -sL https://api.github.com/repos/roflmuffin/CounterStrikeSharp/releases/latest | jq '.assets[]')
cssharplatestfile=$(echo -e "${cssharplastbuild}" | jq -r '.name')
cssharpdownloadurl=$(echo -e "${cssharplastbuild}" | jq -r '.browser_download_url')
